    ...
    navigator.geolocation.getCurrentPosition(userLocation => {
        ...
        reportCol.get().then((reportColData) => {
            reportColData.forEach((reportDoc) => {
                ...
                const latDst = (userLocation.coords.latitude - reportDocData.location[0]) * 110.574;
                const lonDst = (userLocation.coords.longitude - reportDocData.location[1]) * 111.320 * Math.cos(latDst * (Math.PI/180));
                const dst = Math.sqrt(Math.pow(latDst, 2) + Math.pow(lonDst, 2)); console.log(dst)
                if (dst <= MAX_DST) {
                    nearbyList.push([dst, reportDocData]);
                }
            })
        }).then(() => {
            nearbyList.sort(function (a, b) {return a[0] - b[0]});
...
function showReportsInNearbyList() {
    if (nearbyList.length == 0) {return;};

    var html = "";
    nearbyList.forEach(function(listItem) {
        const reportDocData = listItem[1];
        var dstStr;
        if (listItem[0] < 1) {dstStr = `${listItem[0].toFixed(3) * 1000} m`;} else {dstStr = `${listItem[0].toFixed(2)} km`;}
